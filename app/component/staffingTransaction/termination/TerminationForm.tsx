...
export default function TerminationForm({
  ...
  const { data: reason } = useGetDataQuery("termination-reason");
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<TerminationType>({
    mode: "onBlur",
    resolver: zodResolver(
      getTerminationSchema(staffingTransaction.transactionDetails.resignation),
    ),
    defaultValues: {
      ...defaultValues,
        ...
        <Stack direction={"row"} gap={3}>
          {
            <SelectDataControl
              ...
              name="reason"
              data={
                (reason as DropdownOption[])?.map((item) => item.name) || []
              }
              inputLabel={
