...
export const WorkStatusFilter: FC<Props> = ({ control, labels }) => {
  const { statusClassifications: workStatusList } = useAppSelector(
    (state) => state.employeeTable.referenceData,
  );
      ...
      render={({ field }) => (
        <FormControl>
          ...
          <Select
            ...
            placeholder="Select"
            input={
              <OutlinedInput notched label={labels.statusClassification} />
            }
            renderValue={(selected) =>
              workStatusList
                .filter((stat) => selected.includes(stat.id))
                .map((stat) => stat.name)
                .join(", ")
            }
          >
