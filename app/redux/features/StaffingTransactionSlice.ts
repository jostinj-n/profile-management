...
export const fetchStaffingTransactionsByEmployeeId = createAsyncThunk<
  StaffingTransactionResponse,
  string,
  { state: RootState }
>(
  "staffingTransactions/fetchStaffingTransactionsByEmployeeId",
  async (employeeId: string, thunkApi) => {
    try {
      ...
      const response = await fetch(
        `/api/staffingTransactions/${employeeId}?${params.toString()}`,
      );
...
const staffingTransactionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchStaffingTransactionsByEmployeeId.fulfilled,
      ...
      .addCase(
        fetchStaffingTransactionsByEmployeeId.rejected,
        (state, action) => {
          state.isLoading = false;
          state.error =
            action.error.message || "Failed to fetch transactions for employee";
        },
      );
  },
});
...
export const {
  actions: staffingTransactionActions,
  reducer: staffingTransactionReducer,
} = staffingTransactionSlice;
